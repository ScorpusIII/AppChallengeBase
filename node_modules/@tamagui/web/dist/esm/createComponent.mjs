...
import { groupCollapsed, groupEnd } from "./helpers/consoleLog.mjs";
import { getShorthandValue } from "./helpers/getShorthandValue.mjs";
...
import { wrapStyleTags } from "./helpers/wrapStyleTags.mjs";
...
If you meant to do this, you can disable this warning - either change untilMeasured and group at the same time, or do group={conditional ? 'name' : undefined}`), process.env.NODE_ENV === "development" && time && time`destructure`, groupName && (nonTamaguiProps.onLayout = composeEventHandlers(nonTamaguiProps.onLayout, e => {
...
` && console.error(`Unexpected text node: ${item}. A text node cannot be a child of a <${staticConfig.componentName || propsIn.tag || "View"}>.`, props);
    ...
    process.env.NODE_ENV === "development" && isWeb && useIsomorphicLayoutEffect(() => {
      ...
      let tm;
      if (state.unmounted) {
        if (animationDriver?.supportsCSSVars ?? config?.animations?.supportsCSSVars) return tm = setTimeout(() => {
          setStateShallow({
            unmounted: !1
          });
        }), () => clearTimeout(tm);
      ...
      const dispose = pseudoGroups || mediaGroups ? subscribeToContextGroup({
        componentContext,
        setStateShallow,
        state,
        mediaGroups,
    ...
    if (("group" in props || propsIn.focusWithinStyle) && (content = /* @__PURE__ */jsx(ComponentContext.Provider, {
      ...componentContext,
      groups: subGroupContext,
      setParentFocusState: setStateShallow,
      children: content
    })), process.env.NODE_ENV === "development" && time && time`group-context`, content = disableTheme ? content : getThemedChildren(themeState, content, themeStateProps, !1, stateRef), process.env.NODE_ENV === "development" && time && time`themed-children`, isReactNative && !asChild && (content = /* @__PURE__ */jsx("span", {
