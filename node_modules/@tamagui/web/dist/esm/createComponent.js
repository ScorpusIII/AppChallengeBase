  ...
  const component = React.forwardRef((propsIn, forwardedRef) => {
    ...
    if ("themeInverse" in props && (themeStateProps.inverse = props.themeInverse), "theme" in props && (themeStateProps.name = props.theme), typeof curStateRef.isListeningToTheme == "boolean" && (themeStateProps.needsUpdate = () => !!stateRef.current.isListeningToTheme), process.env.NODE_ENV === "development" && debugProp && debugProp !== "profile") {
      ...
      if (console.info(
        ...
        childLog.length && (childLog = `(children: ${childLog})`), groupCollapsed(`${childLog} Props:`), log("props in:", propsIn), log("final props:", props), log({ state, staticConfig, elementType, themeStateProps }), log({ contextProps: styledContextProps, overriddenContextProps }), log({ presence, isAnimated, isHOC, hasAnimationProp, useAnimations }), groupEnd();
      }
    }
...
` && console.error(
    ...
    if (("group" in props || propsIn.focusWithinStyle) && (content = /* @__PURE__ */ jsx(
    ...
    )), process.env.NODE_ENV === "development" && time && time`group-context`, content = disableTheme ? content : getThemedChildren(themeState, content, themeStateProps, !1, stateRef), process.env.NODE_ENV === "development" && time && time`themed-children`, isReactNative && !asChild && (content = /* @__PURE__ */ jsx(
      "span",
      {
